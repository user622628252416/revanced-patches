...
import app.revanced.util.gms.Constants.ACTIONS
import app.revanced.util.gms.Constants.AUTHORITIES
import app.revanced.util.gms.Constants.GMS_VENDOR
import app.revanced.util.gms.Constants.PERMISSIONS
...
/**
 * Helper class for applying bytecode patches needed for the _gms-core-support_ patches.
 */
internal object GmsCoreBytecodeHelper {
    /**
     * Hook a method to check the availability of GmsCore.
     *
    ...
    /**
     * Prime method data class for the [GmsCoreBytecodeHelper] class.
     *
    ...
    /**
     * Patch the bytecode to work with GmsCore.
    ...
    fun patchBytecode(
        ...
        val allTransforms = arrayOf(
            GmsCoreBytecodeHelper::commonTransform,
            GmsCoreBytecodeHelper::contentUrisTransform,
    ...
    private fun commonTransform(referencedString: String): String? =
        when (referencedString) {
            ...
            in PERMISSIONS,
            in ACTIONS,
            in AUTHORITIES -> referencedString.replace("com.google", GMS_VENDOR)

            // subscribedfeeds has no vendor prefix for whatever reason...
            "subscribedfeeds" -> "${GMS_VENDOR}.subscribedfeeds"
            else -> null
        }
        ...
        if (str.startsWith("content://")) {
            ...
            for (authority in AUTHORITIES) {
                ...
                if (str.startsWith(uriPrefix)) {
                    return str.replace(
                        uriPrefix,
                        "content://${authority.replace("com.google", GMS_VENDOR)}"
                    )
                }
            }
            ...
            if (str.startsWith(subFeedsUriPrefix)) {
                return str.replace(subFeedsUriPrefix, "content://${GMS_VENDOR}.subscribedfeeds")
            }
        }
